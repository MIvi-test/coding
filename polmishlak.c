...
#define SIZE (32 * 2 + 2) * 2
#define SPACE 32

 // проще сделать так, чем посстоянно передавать из одной функции в другую

void searching(const char *str, short *right, short *left, short *join)
{
    short i = 0;
    while (str[i] != SPACE)
    {
...
void make_str(const char *old_str, char **new_str)
{
    ...
    short left;
    short join;
    searching(old_str, &right, &left, &join);
